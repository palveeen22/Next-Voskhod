        ...
        <div className='bg-white text-black min-h-screen w-full flex justify-between'>
            <div className='w-1/2 flex-col justify-start items-center px-4'>
                ...
                <div className='flex justify-between gap-2'>
                    <div className="w-48 h-58 flex flex-col justify-between gap-2">
                        {car?.images?.map((image, index) => {
                            return (
                                <Image key={index} src={image.image} width={500} height={500} alt={`${car.brand} ${car.model}`} className="rounded-lg object-cover" />
                            );
                        })}
                    </div>
                    <div>
                        {car?.images?.slice(0, 1).map((image, index) => (
                            <Image key={index} src={image.image} width={600} height={600} alt={`${car.brand} ${car.model}`} className="rounded-lg object-cover" />
                        ))}
                    </div>
                </div>
                    ...
                    >
                        {car?.images?.map((image, index) => (
                            <SwiperSlide key={index}>
                                <Image key={index} src={image.image} width={500} height={500} alt={`${car.brand} ${car.model}`} className="object-cover rounded-lg" />
                            </SwiperSlide>
                        ))}
                    </Swiper>
                </> */}
            ...
            <div className='w-1/2 flex-col justify-center items-center px-4 py-8'>
                ...
                <div className='flex-col justify-center items-center gap-2 m-8'>
                    ...
                    <p className="text-[#484848] font-semibold text-4xl">{formattedNumber(car?.price)}</p>
                </div>
            </div >
